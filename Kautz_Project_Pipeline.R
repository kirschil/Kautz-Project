...
setwd("Q:/Research/2 Current Projects/WIP/State & MSA Recession Forecasting")

fredr_set_key("f71c3c11e9a38e7361a8f04ca2c6b889")

# Updated for graph on app
...
if(addlmonths==2){
  ab<- as.data.frame(rep(as.yearmon(Sys.Date() %m+% months(-1), format="%Y-%m-%d"), times=50))
  ab2<- as.data.frame(rep(as.yearmon(Sys.Date() %m+% months(-2), format="%Y-%m-%d"), times=50))
  othermonthdata<- as.data.frame(cbind(ab, bc, ef, ef))
  othermonthdata2<- as.data.frame(cbind(ab2, bc, ef, ef))
  names(othermonthdata)[1]<- "ab"
  names(othermonthdata2)[1]<- "ab"
  othermonthdata<- as.data.frame(rbind(othermonthdata, othermonthdata2))
  names(othermonthdata)<- c(names(masterstate))
...
for(i in 4:length(statesdata$State)){
  if(is.na(statesdata$Coincident_Lag_2[i])){
    statesdata$Coincident_Lag_2[i] <- statesdata$Coincident_Lag_2[i-1]}
  
  if(is.na(statesdata$Recession_Lag_8[i])){
    statesdata$Recession_Lag_8[i] <- statesdata$Recession_Lag_8[i-1]}
}


#############################################################################################
# Real Trade-Weighted Dollar (FRED)

rtw <- read.csv("rtw.csv", header = TRUE)
states<- names(rtw)
states<- states[-1]
states2<- as.data.frame(substr(states, 6, 7))
names(states2)<- "ab"
...
states2 <- left_join(states2, colp, by="ab")
states2<- states2[-1]
states2<- as.vector(states2$st)

fredr_set_key("f71c3c11e9a38e7361a8f04ca2c6b889")

test<- fredr_series_observations(
  series_id = states[1],
  observation_start = as.Date("1990-02-01"),
  frequency = "m",
  units="lin")
...
test<- test %>% 
  select(date, value)
names(test)<- c("date", states2[1])


for(i in 2:50){
  test2<- fredr_series_observations(
    series_id = states[i],
    observation_start = as.Date("1990-02-01"),
    frequency = "m",
    units="lin"
  ...
  test2<- test2 %>% 
    select(date, value)
  names(test2)<- c("date", states2[i])
  
  test<<- left_join(test, test2, by="date")
}
...
test <- data.frame(diff(as.matrix(test)))    
test <- as.data.frame(cbind(test2$date[-1], test))    
names(test)<- c("date", states2)


rtw <- melt(test, id.vars="date")
names(rtw)<- c("Date", "State", "Trade_Weighted_Dollar")

rtw<- rtw %>% 
  mutate_if(is.factor, as.character) %>% 
  mutate(Date= as.yearmon(Date, format="Y-m-d"))

statesdata <- left_join(statesdata, rtw, by= c("Date", "State"))

for(i in 4:length(statesdata$State)){
  if(is.na(statesdata$Trade_Weighted_Dollar[i])){
    statesdata$Trade_Weighted_Dollar[i] <- statesdata$Trade_Weighted_Dollar[i-1]}
}
...
url2 <- "https://www.philadelphiafed.org/-/media/research-and-data/regional-economy/indexes/leading/leading-revised.xls?la=en"
download.file(url = url2, mode = "wb", destfile = "stateleadingdata.xls")
statelead<- read_excel("stateleadingdata.xls")

statelead<- melt(statelead, id="Date")
names(statelead)<- c("Date", "ab", "State_Leading_Index")
statelead <- left_join(statelead, colp, by="ab")
statelead<- statelead[c(1,4,3)]
names(statelead)[2]<- "State"
statelead <- statelead %>% 
  mutate(Date= as.yearmon(Date, format="%Y-%m-%d")) %>% 
  filter(State!="United States")

statesdata <- left_join(statesdata, statelead, by= c("Date", "State"))

for(i in 4:length(statesdata$State_Leading_Index)){
  if(is.na(statesdata$State_Leading_Index[i])){
    statesdata$State_Leading_Index[i] <- statesdata$State_Leading_Index[i-1]}
}
...
Federal_Funds_Rate_Change <- diff(ffr$value)
Date<- as.character(ffr$date)[-1]

ffr <- as.data.frame(cbind(Date, Federal_Funds_Rate_Change)) %>% 
  mutate(Date=as.yearmon(Date, format="%Y-%m-%d"))

statesdata <- left_join(statesdata, ffr, by= "Date")

for(i in 4:length(statesdata$Federal_Funds_Rate_Change)){
  if(is.na(statesdata$Federal_Funds_Rate_Change[i])){
    statesdata$Federal_Funds_Rate_Change[i] <- statesdata$Federal_Funds_Rate_Change[i-1]}
}
statesdata <- statesdata %>% 
  mutate(Federal_Funds_Rate_Change = as.numeric(as.character(Federal_Funds_Rate_Change)))

########################################################################################################
# State Dataset for Forecasting
...
for(i in 1:length(msadatarest$MSA)){
  if(is.na(msadatarest$Coincident_Lag_5[i])){
    msadatarest$Coincident_Lag_5[i]<- msadatarest$Coincident_Lag_5[i-1]
  }
}

for(i in 1:length(msadatarest$MSA)){
  if(is.na(msadatarest$Coincident_Lag_6[i])){
    msadatarest$Coincident_Lag_6[i]<- msadatarest$Coincident_Lag_6[i-1]
  }
}

for(i in 1:length(msadatarest$MSA)){
  if(is.na(msadatarest$Recession_Lag_10[i])){
    msadatarest$Recession_Lag_10[i]<- msadatarest$Recession_Lag_10[i-1]
  }
}

for(i in 1:length(msadatarest$MSA)){
  if(is.na(msadatarest$Recession_Lag_11[i])){
    msadatarest$Recession_Lag_11[i]<- msadatarest$Recession_Lag_11[i-1]
  }
}
...
msalist <- unique(msaboostdata$MSA)


statelist <- c("New York", "New Mexico", "Pennsylvania", "Georgia", "Texas", "California",
               "Maryland", "Alabama", "Massachusetts", "New York", "North Carolina", "Illinois",
               "Ohio", "Ohio", "South Carolina", "Ohio", "Texas", "Colorado",
               "Michigan", "Michigan", "North Carolina", "South Carolina", "Connecticut", "Texas",
               "Indiana", "Florida", "Missouri", "Nevada", "Arkansas", "California",
               "Kentucky", "Tennessee", "Florida", "Wisconsin", "Minnesota", "Tennessee",
               "Connecticut", "Louisiana", "New York", "Connecticut", "Oklahoma", "Florida",
               "Pennsylvania", "Arizona", "Pennsylvania", "Oregon", "Rhode Island", "North Carolina",
               "Virginia", "California", "New York", "California", "Utah", "Texas", 
               "California", "California", "California", "Florida", "Washington", "Missouri",
               "Florida", "Arizona", "Oklahoma", "California", "Virginia", "Maryland", 
               "Massachusetts")
msastate<- as.data.frame(cbind(msalist, statelist))
names(msastate)<- c("MSA", "State")

msaboostdata<- left_join(msaboostdata, msastate, by="MSA")

mainmsa<- left_join(msaboostdata, statesdata, by=c("State", "Date"))
mainmsa<- mainmsa[c(1, 2, 25, 4:22, 28:37, 39:42, 66:68, 23:24)]
names(mainmsa)[4:36]<- c("Employment_Change_MSA", "MSA_Coincident", "MSA_Recession", "MSA_Coincident_Lag_4",
                         "MSA_Coincident_Lag_5", "MSA_Coincident_Lag_6", "MSA_Coincident_Lag_7", "MSA_Coincident_Lag_8",
                         "MSA_Coincident_Lag_9", "MSA_Coincident_Lag_10", "MSA_Recession_Lag_10", "MSA_Recession_Lag_11",
                         "MSA_Cluster_2", "MSA_Cluster_3", "MSA_Cluster_4", "MSA_Cluster_5", "Brent_Crude_Price", 
                         "Consumer_Sentiment", "US_Leading_Index", "State_Coincident_Lag_2", "State_Coincident_Lag_3", 
                         "State_Coincident_Lag_4", "State_Coincident_Lag_5", "State_Coincident_Lag_6", "State_Coincident_Lag_7", 
                         "State_Coincident_Lag_8", "State_Recession_Lag_8", "State_Recession_Lag_9",
                         "State_Recession_Lag_10", "Employment_Change_State", "State_Cluster_2", "State_Cluster_3", "State_Cluster_4")

msaboostdata<- mainmsa
